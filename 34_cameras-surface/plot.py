...
# plot the disparity field of the rectified camera pair
def plot_disp_field(Di):
    # replace all Di nans with min values (for plot)
    nan_mask = np.isnan(Di)
    Di[nan_mask] = Di[~nan_mask].min()
    plt.imshow(Di, cmap='jet')
    plt.colorbar()
    plt.show()
